...
import { hash10, poseidonHashFromCircom } from '../src/compat/poseidon.mjs';
...
const batch1FixturePath = fileURLToPath(
  new URL(
    '../../amaci-operator/test-data/data/dora124w3vdmqtrjms9k4yhquqrd4r3qx5xww36ay5dg9wn8mnwe2e7dq5v8qfl/rust-inputgen/msg-tally/tally_inputs/000001.json',
    import.meta.url,
  ),
);
const batch2FixturePath = fileURLToPath(
  new URL(
    '../../amaci-operator/test-data/data/dora124w3vdmqtrjms9k4yhquqrd4r3qx5xww36ay5dg9wn8mnwe2e7dq5v8qfl/rust-inputgen/msg-tally/tally_inputs/000002.json',
    import.meta.url,
  ),
);
...
test('validates the existing AMACI TallyVotes fixture', () => {
...
test('validates alternate existing AMACI TallyVotes batches without changing them', () => {
...
test('computes the contract-compatible inputHash', () => {
  const input = loadFixture();
...
test('pure BN254 Poseidon matches the existing circom package', () => {
  const vectors = [
    [0n, 0n],
    [1n, 2n],
...
test('emits stable canonical public output with split uint256 values', () => {
  const input = loadFixture();
...
test('builds fixed-size Cairo program input for the migrated tally relation', () => {
  const input = loadFixture();
...
test('serializes Cairo executable arguments for scarb execute', () => {
  const input = loadFixture();
  const evaluated = evaluateTallyVotes(input);
  const cairoInput = buildCairoTallyInput(input, evaluated);
...
test('cairo model accepts the generated tally program input', () => {
  const input = loadFixture();
  const evaluated = evaluateTallyVotes(input);
  const cairoInput = buildCairoTallyInput(input, evaluated);
...
test('exports stable hash vectors for Cairo hash-gate implementation', () => {
  const input = loadFixture();
  const evaluated = evaluateTallyVotes(input);
  const cairoInput = buildCairoTallyInput(input, evaluated);
...
test('cairo model rejects a tampered hash-gate preimage', () => {
  const input = loadFixture();
  const evaluated = evaluateTallyVotes(input);
  const cairoInput = buildCairoTallyInput(input, evaluated);
...
test('cairo model rejects a tampered hash-gate output', () => {
  const input = loadFixture();
  const evaluated = evaluateTallyVotes(input);
  const cairoInput = buildCairoTallyInput(input, evaluated);
  const programInput = structuredClone(cairoInput.program_input);
...
test('cairo model rejects a tampered packedVals witness', () => {
  const input = loadFixture();
  const evaluated = evaluateTallyVotes(input);
  const cairoInput = buildCairoTallyInput(input, evaluated);
  const programInput = structuredClone(cairoInput.program_input);
...
test('rejects a tampered new tally commitment', () => {
  const input = loadFixture();
...
test('rejects tampered vote leaves', () => {
  const input = loadFixture();
...
test('can calculate Integrity-style fact hash when starknet.js is available', { skip: !isIntegrityHashingAvailable() }, () => {
  const input = loadFixture();
